...
import keras as ks
from keras.layers import Input, Dense, Conv1D, MaxPooling1D, Flatten, Dropout, LSTM
...
def CNN(ini, setup, inp):   
    ...
    #Dense Layer
    dense = Dense(setup["dense_size"], activation='relu', name='dense_gwl')(Flat)
...
def lstm(ini, setup, inp):
    #seed
    np.random.seed(42 + ini)
    tf.random.set_seed(42 + ini)

    # Input Layer
    inp_layer = Input(shape=(setup['seq_length'], inp.shape[2]), name='input')

    # LSTM Layer
    lstm = LSTM(units=setup['lstm_units'], return_sequences=False, name='lstm_1')(inp_layer)

    # Dense Layer
    dense = Dense(setup["dense_size"], activation='relu', name='dense_gwl')(lstm)
    dense = Dropout(0.2, name='dropout_2')(dense)
...
#################### Optimizers 
    optimizer = ks.optimizers.Adam(
        ...
        epsilon=10E-3,
        clipnorm=setup["clip_norm"],
        clipvalue=setup["clip_value"]
    )
