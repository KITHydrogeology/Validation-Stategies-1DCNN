# -*- coding: utf-8 -*-
"""
Created on Fri Jan 31 15:39:22 2025
...
with tf.device("/gpu:0"): 
    ...
    for well in range(50):
        ...
        train_epochs_cv = int(results_cv.mean(axis=0))
        train_epochs_oos = int(results_cv.iloc[4])        
        results_cv.name = Well_ID  
        
                
        ###RepHO
        results_repHO = val_test.rep_HO_validation(Exp, Well_ID, seeds, data, X_train, Y_train, setup) 
        train_epochs_repHO = int(results_repHO.mean(axis=0))
